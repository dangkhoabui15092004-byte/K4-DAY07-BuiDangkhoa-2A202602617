...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for part in parts:
            candidate = temp + (sep if temp else "") + part
...
class HeadingChunker:
    """
    Split text based on Markdown headings (#, ##, ###) and regulation sections (Điều, Mục).
    If a section exceeds max_chunk_size, it subdivides gracefully.
    """
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        for sec in sections:
            if len(sec) <= self.max_chunk_size:
                ...
                for i in range(0, len(sec), self.max_chunk_size):
                    chunk = sec[i : i + self.max_chunk_size].strip()
                    if chunk:
                        chunks.append(chunk)
        return chunks
